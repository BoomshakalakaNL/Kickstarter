...
beforeEach( async () => {
  ...
  const address = await factory.methods.getDeployedCampaings().call();
...
describe('Campaigns', () => {
  ...
  it('requires a minimum amount of contributes', async () => {
    try {
      await campaign.methods.contribute().send({
        value: "1",
        from: accounts[1]
  ...
  it('processes requests', async () => {
    ...
    await campaign.methods.createRequest('buy batteries', web3.utils.toWei('10', 'ether'), accounts[2]).send({
      from: accounts[0],
      gas: '1000000'
    });
